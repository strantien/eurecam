...
directionChangeTolerance0 = 2 # Tolerance  pour comparer le nombre de changements de sense d'une trajectoire.
noiseEqTolerance0 = 30 # Tolerance utlisée pour montrer egalité entre un point d'une image et un point qu'on sait c'est du bruit.
#==============================================================================#
#                      Initialization
#==============================================================================#
...
def trajectory(trajets,Y):
    if len(trajets) == 0 and Y.shape[0] == 0:
        return([])
    if len(trajets) == 0 and Y.shape[0] > 0:
        Ylist = list(Y)
        return([t.nouveauTrajet(p) for p in Ylist])
    if len(trajets) > 0 and Y.shape[0] == 0:
        return(trajets)
    else:
...
def cleanPoints(Y, noiseList, duplicateTol, noiseEqTolerance): #Prende un array et donne un array comme réponse (et la nouvelle liste de noise!)
    if Y.shape[0] == 0:
        ...
        if noNoiseY.shape[0] == 0:
            return(Y, noiseList)
        else:
            YClean.append(tuple(noNoiseY[0]))
            for i in range(noNoiseY.shape[0]):
                if not t.isIn(noNoiseY[i], YClean, t.peopleEqualityFunc, duplicateTol):
...
affichage.plotComplete(dataset, trajectoiresMortes0)
