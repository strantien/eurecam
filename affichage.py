...
#os.chdir("/homes/doua/trantien/documents/doctorat/challenge_amies/Challenge_AMIES_EURECAM/")
            ...
            for k in range(len(traj)):
                if traj[k][0] == i:
                    for j in range(k):
                        ...
                        plt.plot([ix_trajJ, ix_trajJPlus1], [iy_trajJ, iy_trajJPlus1], color = colorTraj, linewidth = .9)
                        #if j > 0:
                        if j == k-1:
                            plt.arrow(0.5 * (ix_trajJ + ix_trajJPlus1), 0.5 * (iy_trajJ + iy_trajJPlus1), 0.5 * (ix_trajJPlus1 - ix_trajJ), 0.5 * (iy_trajJPlus1 - iy_trajJ), shape='full', lw=0, length_includes_head=True, head_width=10, color = colorTraj, label = 'trajectoire' + str(trajectoriesFilm.index(traj)))
        plt.legend()
        plt.savefig('plotdataset001/dataset' + dataset + '_snap' + str(snapNumber + 1))
        snapNumber += 1
